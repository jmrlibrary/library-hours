<head>
<title>JMRL Hours Tool--Alert Messages</title>
...
<!--If 'One message for all libraries' is checked, do not show individual libraries-->
<script type="text/javascript" language="javascript">
	...
	/*Because I guess I didn't know about jQuery yet?  That's what I think the following is for. */
		if (typeof document.getElementsByClassName != 'function') {
...
<?php
//Connect to database
$con = mysql_connect("localhost","jmrl_hoursuser","Ziz(!]2!7@kJ!9kgP#"); 
if (!$con) {
	die('Could not connect: ' . mysql_error());}
mysql_select_db("jmrl_hours", $con);
//
//Functions:
...
$alllibs = array(array(1, 'Central Library'),array(2, 'Crozet Library'), array(3, 'Gordon Avenue Library'), array(4, 'Greene County Library'), array(5, 'Louisa County Library'), array(6, 'Nelson Memorial Library'), array(7, 'Northside Library'), array(8, 'Scottsville Library'), array(9, 'Bookmobile'), array(10, 'Homepage'));
//Process input
//New holiday:
if ($_POST['oldholdate'] == 'new') {
	...
	if ($_POST['closedorset:new'] == 'closed') {
		foreach ($alllibs as $library) {
			...
			$addallclosedresult = mysql_query($addallclosedquery);
		...
		$checkholresult = mysql_query($checkholquery);
		if (mysql_fetch_row($checkholresult) != NULL) {
			...
			foreach ($alllibs as $library) {
				...
				if ($_POST['regularorspecial:new:' . $library[0]] == special) {
					...
					$addholresult = mysql_query($addholquery);
					if ($addholresult) {
						$successes[] = $library[1];
					}
					else {
...
elseif ($_POST['oldholdate'] != NULL) {
	...
	if ($_POST['closedorset:' . $olddate] == 'closed') {
		...
		foreach ($alllibs as $library) {
			...
			$checkholresult = mysql_query($checkholquery);	
			$specialID = mysql_fetch_row($checkholresult);
			if ($specialID[0] != NULL) {
			...
			$updateallclosedresult = mysql_query($updateallclosedquery);
		...
		foreach ($alllibs as $library) {
			...
			if ($_POST['regularorspecial:' . $olddate . ':' . $library[0]] == special) {
				...
				if ($specialID[0] != NULL) {	
					...
					$updateholresult = mysql_query($updateholquery);
					if ($updateholresult) {
						$successes[] = $library[1];
						$succeeded = 1;	
					}
				...
				if ($specialID != NULL) {
					...
					$deleteholresult = mysql_query($deleteholquery);
					if ($deleteholresult) {
						$successes[] = $library[1];
						$succeeded = 1;	
					}
...
//Get Holidays
$alertnamequery = "select distinct Name,StartDate,EndDate from Alert order by StartDate";
$alertnameresult = mysql_query($alertnamequery);
while ($row = mysql_fetch_array($alertnameresult)) {
	$alerts[] = array(stripslashes($row['Name']), $row['StartDate'], $row['EndDate']);}
//Right side menu bar
echo '<br/><div class="col2" style="padding:5px;background-color: #f9f9f9;width:23%;border:1px solid grey;">';
echo '<a onclick="javascript:;pickdiv(\'new\');"><b>Add a New Alert</b></a>';
echo '<br/><br/>Existing alerts:<ul>';
foreach ($alerts as $alert) {
	echo '<li><a  onclick="javascript:;pickdiv(\'' . $alert[0] . $alert[1] . $alert[2] . '\');" ><b>' . $alert[0] . '</b><br>' . usdate($alert[1]) . ' - ' . usdate($alert[2]) . '</a></li>';
	}
echo '</ul>';
echo '</div>';
...
echo '<H3>Add a new Alert Message</H3>';
echo '<table style="width:100%;"><tr style="background-color:#f9f9f9;"><form name="newholiday" method="post" action="alertmessage.php"><td style="padding:10px;width:25%;"><input type="hidden" name="oldholdate" value="new">Name: <input type="text" name="newname" size="20"/></td><td>Start Date: <input type="text" name="newstartdate" class="date" id="newstartdateid"/></td><td>End Date: <input type="text" name="newenddate" class="date" id="newenddateid"/></td>';
...
echo '<label for="newallclosed">Same message on Home Page and Branch Pages</label><br/><input class="closedorset" id="newset" type="radio" name="closedorset:new" value="sethours" onclick="javascript:;showlibraries(\'new\');"';
echo '/><label for="newset">Set messages for each branch page</label><br/></td><td></td></tr>';
...
echo '<div class="holdiv" id="div:' . $holiday[1] . '" style="display:none;">';
	...
	while ($row = mysql_fetch_array($holidayresult)) {
		$libraries[] = $row['LibID'];
		$closed[] = $row['Closed'];
